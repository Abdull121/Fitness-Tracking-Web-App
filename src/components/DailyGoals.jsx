...
import { Flame, Footprints, Clock } from 'lucide-react'
 ...
 function DailyGoals({
  ...
  const onSubmit = (data) => {
    ...
    if (data.stepsTaken > data.targetSteps) {
      ...
      if (data.caloriesBurned > data.outOfCaloriesBurned) {
        ...
        alert('calories Burn cannot exceed out of out of calories. Adjusting value.');
        return;
      }

      if (data.SpendWorkoutTime > data.outOfWorkoutTime) {
        setValue('SpendWorkoutTime', data.outOfWorkoutTime);
        alert('Spend workout time cannot exceed out of workout time. Adjusting value.');
        return;
      }
  ...
  const workoutProgress = outOfWorkoutTime > 0
  ...
  React.useEffect(() => {
    if (stepsTaken > targetSteps) {
      setError("stepsTaken", { type: "manual", message: "Steps cannot exceed the target Steps" });
    } else {
      clearErrors("stepsTaken");
    }
    if(caloriesBurned> outOfCaloriesBurned){
      setError("caloriesBurned", {type: "value", message:"calories Burned  cannot exceed the out of Burned calories"});
    ...
    if (SpendWorkoutTime > outOfWorkoutTime) {
        setError("SpendWorkoutTime", { type: "manual", message: "Workout time cannot exceed the total workout time" });
      } else {
        clearErrors("SpendWorkoutTime");
      }


  }, [stepsTaken, targetSteps,SpendWorkoutTime, outOfWorkoutTime, caloriesBurned,outOfCaloriesBurned, setError, clearErrors]);
    ...
    <div className="w-full max-w-6xl mt-10 mx-auto bg-white p-6 rounded-lg shadow-xl overflow-hidden">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        <div>
          ...
          />
          {errors.caloriesBurned && <p className="mt-1 text-sm text-red-500">{errors.caloriesBurned.message}</p>}
        </div>
        ...
        <label htmlFor="outOfCaloriesBurned" className="block text-sm font-medium text-gray-700 mb-1">
          ...
          <input 
            ...
            className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
          />
          {/* {errors.outOfCaloriesBurned && <p className="mt-1 text-sm text-red-500">{errors.outOfCaloriesBurned.message}</p>} */}
          ...
          <div className="w-full bg-gray-200 rounded-full h-2.5">
            <div 
              ...
              style={{ width: `${caloriesProgress }%`, transition:'width 0.5s ease-in-out' }}
            ></div>
          </div>
        ...
        <div>
          ...
          <input
            ...
            className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
          />
          {errors.stepsTaken && <p className="mt-1 text-sm text-red-500">{errors.stepsTaken.message}</p>}
        </div>
        ...
        <div>
          ...
          <input
            ...
            className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
          />
          {errors.targetSteps && <p className="mt-1 text-sm text-red-500">{errors.targetSteps.message}</p>}
        </div>
        ...
        <div className="mt-2">
          ...
          <div className="w-full bg-gray-200 rounded-full h-2.5">
            <div 
              className="bg-blue-600 h-2.5 rounded-full" 
              style={{ width: `${stepsProgress }%`, transition:'width 0.5s ease-in-out' }}
            ></div>
          </div>
        ...
        <div>
          ...
          <input
            ...
            {...register("SpendWorkoutTime", { 
              ...
              min: { value: 1, message: "Workout time must be a positive number" },
              valueAsNumber: true,
            })}
            className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
          />
          {errors.SpendWorkoutTime && <p className="mt-1 text-sm text-red-500">{errors.SpendWorkoutTime.message}</p>}
        </div>
        ...
        <div>
          ...
          <input
            ...
            className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
          />
          {errors.outOfWorkoutTime && <p className="mt-1 text-sm text-red-500">{errors.outOfWorkoutTime.message}</p>}
        </div>
        ...
        <div className="mt-2">
          ...
          <div className="w-full bg-gray-200 rounded-full h-2.5">
            <div 
              className="bg-blue-600 h-2.5 rounded-full" 
              style={{ width: `${workoutProgress}%`,transition:'width 0.5s ease-in-out' }}
            ></div>
          </div>
          <div className="text-sm text-gray-600 mt-1">{SpendWorkoutTime}/{outOfWorkoutTime} minutes</div>
        </div>
